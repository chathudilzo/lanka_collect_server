...
// Expanded to 100 locations (50 per branch)
const allLocations = [
  // Ratnapura Branch (50)
  "Ratnapura Town",
  "Eheliyagoda",
...
const branchConfigs = [
  {
    ...
    adminEmail: "admin.rnp@finance.lk",
    prefix: "RNP",
    locations: allLocations.slice(0, 50), // 50 centers
  },
  {
    branchId: "BR-EMB-01",
    name: "Embilipitiya Branch",
    ...
    adminEmail: "admin.emb@finance.lk",
    prefix: "EMB",
    locations: allLocations.slice(50, 100), // 50 centers
  },
];

// Helper: Get a random date within the last X days
...
const seedDatabase = async () => {
  try {
    await mongoose.connect(process.env.MONGO_URI);
    console.log(
      "🚀 Connection Successful. Starting 90-Day Historical Data Seed...",
    );
    ...
    for (const config of branchConfigs) {
      console.log(
        `\n🏦 Seeding Branch: ${config.name} (50 Centers, 90 Days History)`,
      );
      ...
      // Increased to 5 Collectors per branch (10 centers each)
      const collectors = [];
      for (let i = 1; i <= 5; i++) {
      ...
      console.log(
        "⏳ Generating 3 Months of Customers, Loans, and Daily Receipts...",
      );
      const allCreatedCustomers = [];
      ...
      for (const center of centers) {
        ...
        for (let j = 1; j <= 5; j++) {
          ...
          const customer = await Customer.create({
            ...
            gender: gender,
            location: {
              latitude: 6.6828 + (Math.random() - 0.5) * 0.05,
              longitude: 80.3992 + (Math.random() - 0.5) * 0.05,
            },
            branchId: branch.branchId,
            centerId: center.centerId,
            totalLoansTaken: Math.floor(Math.random() * 3) + 1,
            customerPhoto: `https://randomuser.me/api/portraits/${gender.toLowerCase() === "male" ? "men" : "women"}/${Math.floor(Math.random() * 70)}.jpg`,
          });

          // 90-Day Historical Loan Start Date
          const principal = 30000 + Math.floor(Math.random() * 5) * 5000;
          const total = principal * 1.15;
          const loanStartDate = getRandomDate(90); // Loan started anywhere up to 3 months ago
          ...
          // HISTORICAL RECEIPTS LOGIC
          for (const installment of loan.schedule) {
            const dueDate = new Date(installment.dueDate);

            // If the due date has passed, simulate real-world payment behavior
            if (dueDate <= today) {
              const payProbability = Math.random();

              if (payProbability > 0.15) {
                // 85% paid on time or slightly late
                const paidDate = new Date(dueDate);
                paidDate.setDate(
                  paidDate.getDate() + Math.floor(Math.random() * 3),
                ...
                await Receipt.create({
                  receiptNo: `REC-${loan.loanId}-${paidDate.getTime()}`,
                  loanId: loan._id,
                  customerId: customer._id,
                  collectorId: collector._id,
                  branchId: branch.branchId,
                  ...
                  paymentMethod: "Cash",
                  status: "valid",
                  createdAt: paidDate, // Crucial for 3-month charts!
                  location: customer.location,
                  handoverId: null,
                });
              } else {
                installment.status = "unpaid"; // Leaves arrears for PAR metrics
              }
            }
          }
          await loan.save();
      ...
      console.log("📝 Generating 90-Day Application History...");
      for (const collector of collectors) {
        const centerId = collector.assignedCenters[0];

        // Pending Applications (Only in the last 7 days)
        for (let k = 0; k < 3; k++) {
          await LoanApplication.create({
            nic: `2000${Math.floor(10000000 + Math.random() * 90000000)}`,
            fullName: `Pending Lead ${k + 1}`,
            phone: "077000000" + k,
            address: "123 New Street",
            gender: "Male",
            loanAmount: 50000,
            ...
            branchId: branch.branchId,
            centerId: centerId,
            createdAt: getRandomDate(7), // Very recent
          });
        }

        // Rejected Applications (Spread over 90 days)
        for (let k = 0; k < 4; k++) {
          const rejectedDate = getRandomDate(90);
          await LoanApplication.create({
            nic: `1985${Math.floor(10000000 + Math.random() * 90000000)}V`,
            fullName: `Rejected Applicant ${k}`,
            phone: "075000000" + k,
            address: "Bad Credit Lane",
            gender: "Female",
            loanAmount: 100000,
            status: "rejected",
            reviewedBy: admin._id,
            reviewNote: "Poor credit history.",
            reviewedAt: rejectedDate,
            createdAt: new Date(rejectedDate.getTime() - 86400000), // Created 1 day before review
            collectorId: collector._id,
            branchId: branch.branchId,
            centerId: centerId,
          });
    ...
    console.log(`\n✅ FULL 90-DAY MULTI-BRANCH SEED COMPLETE!`);
    console.log(
      `Log in with: admin.rnp@finance.lk or admin.emb@finance.lk (Pass: admin123)`,
    );
    process.exit();
